...
def evaluate(model, test_loader, original_data_file):
    ...
    # first compute how many events need prediction
    total_events = 0
    for line in data_lines:
        if "pred_events_concept" in line:
            total_events += len(line["pred_events_concept"])
    # ensure test set size matches the JSON file
    assert len(test_loader.dataset) == total_events, \
        f"Data mismatch: test set has {len(test_loader.dataset)} items, but JSON file has {total_events} items"

    with torch.no_grad():
        total_test_batches = len(test_loader)
        update_interval = max(1, int(total_test_batches * 0.5))  # update every 50%
        progress_bar = tqdm(
            test_loader,
        ...
        for batch_idx, (texts, labels) in enumerate(progress_bar):
            ...
            for pred in preds:
                # locate the line and event corresponding to this prediction
                line_idx = 0
                entity_idx = 0
                found = False

                # search which line/event this prediction belongs to
                while line_idx < len(data_lines) and not found:
                    if "pred_events_concept" in data_lines[line_idx]:
                        if pred_idx - entity_idx < len(data_lines[line_idx]["pred_events_concept"]):
                            # found the target slot for this prediction
                            data_lines[line_idx]["pred_events_concept"][pred_idx - entity_idx]["pred_type"] = \
                                id2label[pred]
                            found = True
                        else:
                            entity_idx += len(data_lines[line_idx]["pred_events_concept"])
                            line_idx += 1
                    else:
                        line_idx += 1

                if not found:
                    raise IndexError(f"Cannot find a matching event for prediction index {pred_idx}")
...
def read_data(annotation_file, only_first_concept=False):
    with open(annotation_file, "r") as f:
        ...
        for raw_line in f:
            ...
            if "pred_events_concept" in line.keys():
                concept_key = "pred_events_concept"
            else:
                concept_key = "events"
            ...
            for ent in line[concept_key]:
                if "concept" in ent.keys():
                    ...
                    if only_first_concept:
                        ...
                        sentences.append(sentence)
                        entity_names.append(entity_name)
            golden_ent = [ent for ent in line["events"] if ent["type"] != "Other"]
...
def train_classifier(model, train_loader, test_loader, optimizer, epochs, gold_mention_n, annotation_file_test):
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            #     "processed": 0.2 * (batch_idx + 1) % update_interval,
...
def eval_classifier(model, train_loader, test_loader, optimizer, epochs, gold_mention_n, annotation_file_test, best_model_path):
    for epoch in range(EPOCHS):
        ...
        with open(f"{best_model_path}/predict2.json", 'w') as f:
            for line in data_lines:
                f.write(json.dumps(line) + '\n')
...
def load_model(save_dir, num_layers, hidden_dim, model_name, concept_texts, concept_library, tune_layers):
    ...
    model = NeuralConceptModel(num_layers=num_layers, hidden_dim=hidden_dim, concept_texts=concept_texts,
                               model_name=model_name, num_classes=len(concept_library)).to(device)
...
parser.add_argument('--file_dir', type=str, default="ACE2005")
